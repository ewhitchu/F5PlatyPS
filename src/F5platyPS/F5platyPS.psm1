...
##  4. Add comment "# yield" on subroutine calls that write values to pipeline.
##     It would help keep code maintainable and simplify ramp up for others.
##

Import-LocalizedData -BindingVariable LocalizedData -FileName F5platyPS.Resources.psd1
        ...
        else
        {
            ...
            $iterator | ForEach-Object {
                if ($PSCmdlet.ParameterSetName -eq 'FromModule')
                {
                    ...
                    # Added ERW 7th Mar 23
                    $ModuleDescription = (Get-Module $ModuleName).Description
                    $CmdletObjs = GetCommands -Module $moduleName
                }
                else # 'FromMaml'
                {
                    if (-not (Test-Path $_))
                ...
                if($WithModulePage)
                {
                    ...
                    {
                        $ModuleDescription = $LocalizedData.Description
                    }
                    # yield
    ...
    process
    {
        ...
        else # FromFile)
        {
            GetMarkdownFilesFromPath $Path -IncludeModulePage | ForEach-Object {
                ...
                [Markdown.MAML.Parser.MarkdownParser]::GetYamlMetadata($md) # yield
            }
        }
    }
}
...
function Merge-MarkdownHelp
{
    ...
    end
    {
        ...
        $fileGroups | ForEach-Object {
            ...
            MySetContent -path $outputFilePath -value $md -Encoding $Encoding -Force:$Force # yield
        }
    }
}
...
function NewModuleLandingPage
{
    ...
    process
    {
        ...
        {
            # ERW 7th Mar 23
            # Must be first time create Landing Page
            # so get Metadata and create the landing page
            # lookup descriptions using synopsis from each command

            # initially set MAML content
                $ModuleGuid = $ModuleGuid
                $FwLink = $FwLink
                $Version = $Version
                $Locale = $Locale
        ...
        {
            # modified ERW 7th Mar 23 to get descriptions from command synopses

            #$CmdletNames | ForEach-Object {
            #    $Content += "### [" + $_ + "](" + $_ + ".md)`r`n" + $LocalizedData.Description + "`r`n`r`n"
            #}
            $CmdletObjs | ForEach-Object {
                $Content += "### [" + $_.Name + "](" + $_.Name + ".md)`r`n" + $_.Description + "`r`n`r`n"
            }
        }
