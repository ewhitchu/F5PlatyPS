...
This is a modified version of platyPS Release 2.0.0. Preview.

It has been modified by Fusion5 Pty Ltd to overcome a few difficulties with using it in an Azure Devops environment using Git but not using AppVeyor. 

This version is also intended for a Windows-exclusive environment, and to manage and extend an already extensive set of PowerShell Modules.

Below is the original content of the README from GitHUb as at 7th April 2023.

The modifications have been done using a clone of the original GitHub project, while still using GitHub (as user ewhitchu@live.com.au). The published Module has then been added to a NuGet package and installed into Fusion5's HCM group's Azure Devops development environment.

The project is still incomplete, in that the AppVeyor yml has not been re-written as an Azure Devops pipeline script (yml).
